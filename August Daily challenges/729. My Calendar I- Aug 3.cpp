...
// Approach: store the ranges in the map with their value as +1 for start and -1 for end
// and keep calculating sum.
// if at any time the sum exceedes 1, it is an intersection because before we could close the first event, another event started.
class MyCalendar {
public:
    map<int,int>mp;
    MyCalendar() {
