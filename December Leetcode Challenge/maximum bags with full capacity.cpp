...
public:
    int maximumBags(vector<int>& capacity, vector<int>& rocks, int additionalRocks) {
        int n = capacity.size();
        vector<int>req(n);// vector to store required number of rocks
        for(int i = 0; i<n; i++){
            req[i] = capacity[i]-rocks[i];
        }
        int ans = 0;
        sort(req.begin(),req.end()); // sorting the req vector so that we fill that bag first whose requirement is minimum(because we have to maximize the number of bags that could have full capacity)
        ...
        for(int i = 0; i<n; i++){
            ...
            if(req[i]<=additionalRocks){
                ...
                ans++; // everytime a bag becomes full, increment the ans.
