...
public:
    ...
    string get(string key, int timestamp) {
        if(mp.find(key)==mp.end())return ""; // if key not found
        return (*mp[key].lower_bound(-1*timestamp)).second; // if key found and if the timestamp is present then it will return it's value otherwise it will return largest timestamp_prev value
    }
};
